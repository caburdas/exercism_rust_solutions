...
To run the bonus tests, remove the #[ignore] flag and execute the tests with the big-primes feature, like this:
...
pub fn public_key(p: u64, g: u64, a: u64) -> u64 {
    /*
            ...
            A = g**a mod p
    */
    // g.pow(a.try_into().unwrap()).rem_euclid(p)
    mod_exp(g as u128, a, p)
}

fn mod_exp(mut base: u128, mut exp: u64, modular: u64) -> u64 {
    /*  from wikipedia --> https://en.wikipedia.org/wiki/Modular_exponentiation#Right-to-left_binary_method
    ...
    */
    ...
    while exp > 0 {
        if exp % 2 == 1 {
            result = (result * base) % modular as u128;
        }
        exp = exp >> 1;
        base = (base * base) % modular as u128;
    }
    result as u64
}
pub fn secret(p: u64, b_pub: u64, a: u64) -> u64 {
    ...
    s = B**a mod p
     */
    //b_pub.pow(a.try_into().unwrap()).rem_euclid(p)
